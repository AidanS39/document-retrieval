...
def _test_embed_then_index(
    embedder_cls: Type[ColPageEmbedder],
    model_name: str,
    engine,
    device: torch.device,
    data_dir: Path,
    ...
    """
    Embed pages, unload the embedder, load the indexer, then index the embeddings.

    Uses a temporary directory for embeddings and indexes, with the real models
    directory symlinked in so model weights are not re-downloaded.

    Verifies that all successfully embedded page_ids appear in the FastPlaid index.
    """
...
def test_webai_embed_then_index(model_name: str, device, engine, data_dir, page_ids):
    _test_embed_then_index(WebAIColPageEmbedder, model_name, engine, device, data_dir, page_ids)
...
def test_tomoroai_embed_then_index(model_name: str, device, engine, data_dir, page_ids):
    _test_embed_then_index(TomoroAIColPageEmbedder, model_name, engine, device, data_dir, page_ids)
...
def test_qwen3_5_embed_then_index(model_name: str, device, engine, data_dir, page_ids):
    _test_embed_then_index(Qwen3_5ColPageEmbedder, model_name, engine, device, data_dir, page_ids)
...
def test_nemotron_embed_then_index(model_name: str, device, engine, data_dir, page_ids):
    _test_embed_then_index(NemotronColPageEmbedder, model_name, engine, device, data_dir, page_ids)
